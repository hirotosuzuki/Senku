...
## 歴史的な背景：Iteratorモデル（Volcanoモデル）の誕生

### 1990年代：Goetz Graefeの提案

Iteratorモデルは、1990年代にオレゴン大学のGoetz Graefeによって提案されました。当時、データベースのクエリ実行方法には様々なアプローチがありましたが、Graefeは「各演算子を独立したIteratorとして実装する」というシンプルで強力なアイデアを提唱しました。

このモデルは、後に「Volcanoモデル」と呼ばれるようになりました。なぜなら、各演算子が`next()`メソッドを呼び出すたびに、データが「噴出」するように流れるからです。

### Iteratorパターンの起源

Iteratorパターン自体は、1970年代から存在していました。GoF（Gang of Four）のデザインパターンでも、Iteratorパターンは重要な位置を占めています。

Iteratorパターンの核心は、以下の3つのメソッドです：

1. `open()`: イテレータを初期化
2. `next()`: 次の要素を取得
3. `close()`: イテレータを終了

このシンプルなインターフェースにより、様々なデータソース（配列、リスト、ファイル、ネットワークなど）を統一的な方法で扱うことができます。

### 現代のデータベースでの採用

Iteratorモデルは、現代の多くのデータベースエンジンで採用されています：

- **PostgreSQL**: 実行エンジン全体がIteratorモデルを採用
- **MySQL**: クエリ実行にIteratorモデルを使用
- **SQLite**: 簡易版のIteratorモデルを実装

これらの実績により、Iteratorモデルは「実証済みのアーキテクチャ」として確立されています。
