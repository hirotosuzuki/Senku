...
1960年代の初期のデータベースシステムでは、データをディスクに保存する際、大きな問題に直面していました。データは可変長（文字列の長さが異なる、レコードのサイズが異なる）であることが多く、以下のような課題がありました：

- **ランダムアクセスの困難**: 可変長データでは、N番目のレコードの位置がわからない
- **メモリ管理の複雑さ**: 可変長データをメモリに読み込む際、動的にメモリを確保する必要がある
- **I/O効率の低下**: 小さなデータを読み書きするたびにディスクアクセスが発生
- **断片化の問題**: データの削除や更新により、ディスク上に無駄なスペースが発生

### 1970年代：固定長ブロックの導入

1970年代になると、オペレーティングシステムの世界で「ブロック」という概念が確立されました。ディスクは固定長のブロック（通常512バイトや4KB）に分割され、OSがブロック単位でI/Oを管理するようになりました。

データベースシステムでも、この考え方を応用する動きが始まりました。しかし、OSのブロックサイズは小さすぎるため、データベース独自の「ページ」という概念が生まれました。

### 1980年代：ページベースストレージの標準化

1980年代になると、多くのデータベースシステムが「ページベースストレージ」を採用するようになりました。固定長ページ（通常8KB〜16KB）を使うことで、以下の問題が解決されました：

- **効率的なI/O**: ページ単位で読み書きすることで、ディスクアクセス回数を削減
- **ランダムアクセスの実現**: ページIDがわかれば、`ページID × ページサイズ`で直接アクセス可能
- **メモリ管理の容易さ**: ページ単位でメモリを確保・解放できる
- **キャッシュの効率化**: ページ単位でキャッシュすることで、頻繁にアクセスするページをメモリに保持

可変長データ（文字列など）は、ページ内に「長さ情報 + データ」の形式で保存し、複数のページにまたがる場合は「オーバーフローページ」を使用する方式が確立されました。

### 現代：ページサイズの最適化

現代のデータベースシステムでは、用途に応じてページサイズが最適化されています：

- **PostgreSQL**: 8KBの固定長ページを採用（汎用的なバランス）
- **MySQL (InnoDB)**: 16KBの固定長ページを採用（大規模データに適している）
- **SQLite**: 可変長ページを採用（特殊なケース、組み込み用途）

また、SSDの普及により、ページサイズの最適化も進んでいます。SSDはランダムアクセスが高速なため、より大きなページサイズも検討されています。

これらの実績により、ページベースストレージは「実証済みのアーキテクチャ」として確立されています。フェーズ1では8KBの固定長ページを採用していますが、これはPostgreSQLと同じサイズで、汎用的な用途に適しています。
