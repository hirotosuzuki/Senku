...
1. **INSERT操作時**:
   - タプルをヒープファイルに挿入
   - WALファイルにINSERTログを書き込む（Write-Ahead）
...
sequenceDiagram
    ...
    Note over D,HF: 1. タプルをメモリ上のページに挿入
    D->>HF: insert_tuple(tuple_data)
    HF->>P: insert_tuple(tuple_data)
    P->>P: ページ内のデータ領域に追記<br/>(メモリ上のみ、まだディスクには書き込まない)
    P-->>HF: (page_id=0, slot_id=0)
    HF-->>D: (page_id=0, slot_id=0)
    
    Note over D,WALFile: 2. Write-Ahead: WALファイルに先に書き込む
    ...
    Note over D,DataFile: 3. データファイルへの書き込みは後で行う<br/>(チェックポイント時または必要に応じて)
...
**重要なポイント**:
- **メモリ**: ページにタプルを挿入（即座に反映）
- **WALファイル（ディスク）**: ログを書き込み、`fsync()`で確実に保存（Write-Ahead）
